<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        <span
          ...
          @click="changeNum(item, userAddressList)"
          >{{ item.consignee }}</span
        >
        <p>
    ...
    <div class="sub clearFix">
      ...
      <a href="javascript:" class="subBtn" @click.prevent="submitTradeIndent"
        >提交订单</a
      >
    </div>
  </div>
...
export default {
  name: 'Trade',
  async mounted() {
    await this.$store.dispatch('trade/getTradeInfoAction')
  },
  computed: {
    ...mapGetters('trade', [
      'detailArrayList',
    ...
    ...mapState({ tradeNo: (state) => state.trade.tradeInfo.tradeNo }),
    defaultUserAddress() {
      return (
  ...
  methods: {
    changeNum(current, userAddressList) {
      userAddressList.forEach((element) => {
        element.isDefault = 0
      })
      current.isDefault = 1
    },
    // 提交交易页面数据去生成订单编号跳转支付页面
    async submitTradeIndent() {
      ...
      try {
        const result = await this.$API.reqTradeNumber(tradeNo, orderInfo)
