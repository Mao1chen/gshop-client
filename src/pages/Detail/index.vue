      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- change事件和blur事件都是失去焦点触发，但是change事件里面的值改变了才会调，blur失去就调用 -->
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="skunum"
                  @change="skunum = skunum >= 1 ? Math.ceil(skunum) : 1"
                />
                <!-- 如果skunum大于等于1就返回当前这个数字,否则就返回1,如果是小数就向上取整 -->
              ...
              <div class="add">
                <a href="#" @click.prevent="addShopCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    // 组件一挂载的时候就调用这个方法,然后调用下面的方法分发动作 推入actions
    this.reqSkuDetailInfo()
  },
  computed: {
    // 不映射的话就是 this.$store.state.detail.skuDetailInfo.categoryView
    //下面加上了命名空间,不加就是状态都在里面堆着
  ...
  methods: {
    reqSkuDetailInfo() {
      this.$store.dispatch('detail/reqSkuDetailInfo', this.skuId)
    },
    changeChecked(spuSaleAttrValue, spuSaleAttrValueList) {
      // 排他思想, 所有元素的值都改为0 ,当前元素改为1
    ...
    async addShopCart() {
      /* 一点击添加购物车,我们就去分发(dispath),然后执行里面的函数发送请求,如果成功的话返回'ok',如果失败的话
      返回一个失败的promise,然后这边会捕获到抛出,分发了之后把当前商品信息存储到会话存储(浏览器关闭就会消失),并且编程式路由
      导航到AddCartSuccess组件,并且携带query参数(把商品数量带过去),注册路由那里也映射一下query参数,路由组件
      直接props接收,然后读取会话存储的商品信息并且动态渲染
      */
      try {
        await this.$store.dispatch('shopCart/reqAddOrUpdate')
        alert('添加购物车成功')
